...
from datasets.Denoise_dataset import NPYDataset_SID
...
parser.add_argument('--output_path', type=str, default='./Denoise_results_SID', help="outputs path")
...
parser.add_argument('--synthesis_root', default='/home/dancer/train_datasets/Generated_train_samples_SID/')
...
def forward_patches(model, noisy, clean_raw, patch_size=256 * 3, pad=32):
    ...
    denoised = stack2raw_RGGB(denoised[0]).detach().cpu().numpy()
    clean_raw = stack2raw_RGGB(clean_raw[0]).detach().cpu().numpy()

    denoised = denoised.clip(0, 1)
    clean_raw = clean_raw.clip(0, 1)
    return denoised, clean_raw
...
def main():
    ...
    train_set = NPYDataset_SID(opts.synthesis_root, config['test_img_list'], mode='train', patch_size=opts.patch_size)
    train_loader = torch.utils.data.DataLoader(train_set, batch_size=opts.batch_size, shuffle=True, drop_last=True,
                                               num_workers=0, pin_memory=True)
    val_set = NPYDataset_SID(config['SID_root'], config['test_img_list'], mode='test', patch_size=opts.patch_size)
        ...
        if (epoch + 1) % opts.image_save_epoch == 0:
            ...
            for batch_idx, data in tqdm(enumerate(val_loader), total=len(val_loader), desc='Valid', ncols=80,
                ...
                with torch.no_grad():
                    ...
                    psnr += res['PSNR']
                    ssim += res['SSIM']
                    # psnr += skimage.metrics.peak_signal_noise_ratio(clean_raw, denoise_raw, data_range=1)
                    # ssim += skimage.metrics.structural_similarity(clean_raw, denoise_raw)
                    count += 1
