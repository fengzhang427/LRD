...
def apply_gains(bayer_images, red_gains, blue_gains):
    ...
    gains = torch.stack([red_gains, green_gains, green_gains, blue_gains], dim=-1)    # [B, B, B, B] -> BC

    gains = gains.unsqueeze(0)

    gains = gains[:, None, None, :]   # BC-> BHWC
    outs = bayer_images * gains      # BHWC * BHWC
...
def binning(bayer_images):
    ...
    lin_rgb = torch.stack([
        bayer_images[:, 0, ...],
        torch.mean(bayer_images[:, [1, 2], ...], dim=1),
        bayer_images[:, 3, ...]], dim=1)
    return lin_rgb
...
def process_MIR_SID(bayer_images, red_gains, blue_gains, cam2rgbs):
    """Processes a batch of Bayer RGGB images into sRGB images."""
    # White balance.
    bayer_images = apply_gains(bayer_images, red_gains, blue_gains)
    # Demosaic.
